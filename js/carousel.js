...
window.addEventListener("mousemove", () => {
	...
	productCards.forEach((card) => {
		// populate product modal with information from products slide
		card.addEventListener("click", (e) => {
			...
			const targetTitle =
				targetSlide.querySelector("#overlayTitle").innerText;
			const targetPrice =
				targetSlide.querySelector("#overlayPrice").innerText;

			cardContent.querySelector(
				"h3"
			).innerText = `Rainy Days ${targetTitle} Jacket`;
			cardContent.querySelector("h4").innerText = targetPrice;
			cardContent.querySelector("p").innerText =
				targetSlide.querySelector("img").alt;
